...
<<z-test>>=
z_test(pisa, sel_set[[10]], sel_set[[9]], alternative = "less", v = 1)
@
...
<<corr-iita1>>=
ex_corr <- iita(ex_data$dataset, v = 2)
ex_corr
@
...
<<corr-iita2>>=
ex_corr$implications == ex_data$implications
@
...
<<pop-iita>>=
pop <- pop_iita(ex_data$implications, 0.1, 0.1, 9, dataset = ex_data$dataset, v = 2)
attributes(pop)
@
...
<<diff1>>=
round(ex_corr$diff / 1500^2, 4)
@
<<diff2>>=
round(pop$pop.diff, 4)
@
...
<<pop-relation>>=
mp <- which.min(pop$pop.diff)
pop$selection.set[[mp]] 
@
...
<<est-var>>=
var_sample <- variance(ex_data$dataset, ex_data$implications, v = 2)
var_sample
sqrt(var_sample)
@
...
<<pop-var>>=
pop_variance <- pop_variance(pop$pop.matrix, pop$selection.set[[mp]], pop$error.pop[mp], v = 2)
pop_variance
sqrt(pop_variance)
@
